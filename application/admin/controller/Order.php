...
class Order extends Backend
{
  ...
  public function index()
  {
    ...
    if ($this->request->isAjax()) {
      ...
      //医生账号
      if($group_id == 9){
        $agent_id = Admin::where('id',$this->auth->id)->value('agent_id');
        $query->where('shop_id',$agent_id);
      }
      ...
      foreach ($list as $row) {
        ...
        $arr = array('1'=>'待寄模具','2'=>'待出方案','3'=>'待客户确认','4'=>'待付款','5'=>'周期发货','6'=>'已完结',);
  ...
  public function detail($ids)
  {
    ...
    $order_data = array(
      ...
      'current_cycle'=>$order_info['current_cycle'],
    ...
    $cycle_arr = OrderLog::where('order_id',$order_id)
      ->where('type',5)
      ->field('id,order_id,node,courier_code,courier_id,createtime,name,send_type,phone')
      ->order('id desc')
      ->select();
    ...
    //计算发了几次货
    $count = OrderLog::where('order_id',$order_id)->where('type',5)->group('sign')->count();
  ...
  public function cycle(){
    ...
    if($data['send_type'] == 1){
      //快递
      for($i=0;$i<$data['cyil'];$i++){
        //周期发货可能是快递 或者送货   多期数一起发
...
//周期发货可能是快递 或者送货   多期数一起发
  ...
  public function plan1(){
    $order_id =$this->request->param('order_id');
    $file_url =$this->request->param('images');
